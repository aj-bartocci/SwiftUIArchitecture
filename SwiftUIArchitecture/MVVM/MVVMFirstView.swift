...
@MainActor
class MVVMFirstViewModel: ObservableObject {
...
struct MVVMFirstView: View {
    ...
    struct Content: View {
        @StateObject var viewModel: MVVMFirstViewModel
        
        init(state: GlobalStateObject) {
            self._viewModel = StateObject(wrappedValue: MVVMFirstViewModel(state: state))
        }
