...
@MainActor
class MVVMThirdViewModel: ObservableObject {
...
struct MVVMThirdView: View {
    ...
    struct Content: View {
        @StateObject var viewModel: MVVMThirdViewModel
        
        init(state: GlobalStateObject) {
            self._viewModel = StateObject(wrappedValue: MVVMThirdViewModel(state: state))
        }
